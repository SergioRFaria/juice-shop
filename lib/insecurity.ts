...
import { requiredEnv, requiredPemEnv } from './env'
...
const privateKey = requiredPemEnv('JWT_PRIVATE_KEY')
...
export const hmac = (data: string) => crypto.createHmac('sha256', requiredEnv('HMAC_SECRET')).update(data).digest('hex')
...
export const isRedirectAllowed = (url?: string) => {
  ...
  for (const allowedUrl of redirectAllowlist) {
    ...
    if (
      target.origin === allowedTarget.origin &&
...
export const deluxeToken = (email: string) => {
  const hmac = crypto.createHmac('sha256', requiredEnv('DELUXE_TOKEN_SECRET'))
  return hmac.update(email + roles.deluxe).digest('hex')
}
